...
function updateNav(hash) { $all('.nav-item').forEach(function (el) { el.classList.remove('active'); }); var m = { '#/home': 'home', '#/my-orders': 'order', '#/orders': 'orders', '#/sell': 'seller', '#/profile': 'profile' }; var el = document.querySelector('[data-nav="' + (m[hash] || '') + '"]'); if (el) el.classList.add('active'); }
